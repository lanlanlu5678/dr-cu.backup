...
void PartialRipup::markOfGuides(const vector<int> &nets) {
    // if (!db::rrrIterSetting.addDiffLayerGuides) return;

    // auto mark = [&nets](int id) {
    //     auto &net = database.nets[nets[id]];
    //     auto &guides = net.routeGuides;
    //     for (int i=0; i<guides.size(); i++) {
    //         auto &g = guides[i];
    //         boostBox b(boostPoint(g.x.low, g.y.low), boostPoint(g.x.high, g.y.high));
    //         auto coverRange = boost::make_iterator_range(bgi::qbegin(database.drcMarks[g.layerIdx], bgi::covered_by(b)),
    //                             bgi::qend(database.drcMarks[g.layerIdx]));
    //         // int numVios = int(boost::distance(coverRange));
    //         // net.routeGuideVios[i] = (int(numVios) > 30) ? 1 : 0;
    //         net.routeGuideVios[i] = int(boost::distance(coverRange));
    //     }
    // };

    // runJobsMT(nets.size(), mark);
}

void PartialRipup::expandRouteGuides(const vector<int> &nets, bool oriMode) {
    // std::mutex plock;
    // int e = 0;
    auto expand = [&nets, /*&plock, &e, */oriMode](int id) {
        auto &net = database.nets[nets[id]];
        auto &guides = net.routeGuides;
        // int n = 0;
        for (int i=0; i<guides.size(); i++) {
            if (oriMode || net.routeGuideVios[i]>0) {
                // n++;
                database.expandBox(guides[i], db::rrrIterSetting.defaultGuideExpand);
            }
        }
        // plock.lock();
        // e += n;
    ...
    runJobsMT(nets.size(), expand);
    // log() << "$$$$$$$$$$$$$$$$$$$$$$$$$$ " << e << " times" << std::endl;
}